...
def set_up_directory(directory: Path, extensions: dict[str,list[str]]) -> None:
    ...
    folders_to_ensure: list[str] = list[str](extensions.keys())

    folders_to_ensure.append("Other")

    for folder in folders_to_ensure:

        target_path: Path = directory / folder
...
def move_file(directory:Path, destination_directory:Path) -> None:
    ...
    _ = shutil.move(src=directory, dst=destination_directory)

def organize_directory(directory:Path) -> None:
    for item in directory.iterdir():
        if item.is_file():
            extension: str = item.suffix
            source: Path = directory / item.name
            destination: Path = directory / get_category_for_extension(extension, extensions) / item.name
            move_file(directory=source, destination_directory=destination)
